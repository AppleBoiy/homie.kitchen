...
export async function PUT(request, { params }) {
  try {
    ...
    const updateIngredient = await dbAdapter.prepare(`
      UPDATE ingredients 
      SET name = ?, description = ?, stock_quantity = ?, unit = ?, min_stock_level = ?
      WHERE id = ?
    `);
    
    const result = await updateIngredient.run(name, description, stock_quantity || 0, unit, min_stock_level || 10, id);
...
export async function DELETE(request, { params }) {
  try {
    ...
    const deleteIngredient = await dbAdapter.prepare('DELETE FROM ingredients WHERE id = ?');
    const result = await deleteIngredient.run(id);
